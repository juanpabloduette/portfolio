...
const inputsPortfolio = document.querySelectorAll('#formulario-portfolio input');
...
formularioPortfolio.addEventListener('submit', (e) => {
	...
	if(camposPortfolio.nombrep && camposPortfolio.correop){
		...
		document.getElementById('formulario__mensaje-exito-portfolio').classList.add('formulario__mensaje-exito-activo');
		setTimeout(() => {
			document.getElementById('formulario__mensaje-exito-portfolio').classList.remove('formulario__mensaje-exito-activo');
		}, 5000);
        ...
        document.getElementById('formulario__mensaje-portfolio').classList.add('formulario__mensaje-activo');
		setTimeout(() => {
			document.getElementById('formulario__mensaje-portfolio').classList.remove('formulario__mensaje-activo');
		}, 5000);
...
function changeLanguage() {
	...
	if (languageButton.dataset.language === 'en' || languageButtonMovil.dataset.language === 'en') {
	   languageButton.dataset.language = 'es';
	   languageButtonMovil.dataset.language = 'es';
	   languageButton.textContent = 'EN';
	   languageButtonMovil.textContent = 'EN';
	   element1.innerHTML = '<p id="element1">Hola! &#128075; soy <span>Juan</span>, un desarrollador web de Argentina. Interesado por el diseño, Criptos, Startups y Marketing. Bienvenidos a mi sitio web.</p>';
		home.textContent = 'Inicio';
		aboutme.textContent = 'Sobre mi';
		portfolio.textContent = 'Portfolio';
		business.textContent = 'Tenes un emprendimiento? Queres ofrecer tus servicios con un sitio web?';
		make.innerHTML = '<p id="make">Crear un sitio web para ese fin requiere de estar siempre actualizado y acompañando al cliente para poder lograr el mejor resultado. <br> <br>Los proyectos se realizar para que sean adaptables a todos los dispositivos móviles <strong>( 100% Responsivos )</strong>.</p>';
		one.textContent = 'Diseño Web';
		two.textContent = 'Desarrollo';
		profesional.innerHTML = '<p>Desarrollo profesional para pymes y emprendedores. <br><br> También ofrecemos servicios de <strong>web hosting</strong>.</p>';
		three.innerHTML = '<i class="fa-solid fa-arrow-up"></i>Ver más';
		together.textContent = 'Trabajamos juntos?';
		togethertext.innerHTML = '<p id="togethertext">Nos ponemos en contacto a la brevedad para comenzar el proyecto juntos.<strong>Contactame</strong>.</p>';
		...
		errortext.innerHTML = '<i class="fas fa-exclamation-triangle"></i> Por favor rellena el formulario correctamente.';
		modalone.innerHTML = '<i class="fa-solid fa-calendar-days"></i><h3>Tiempo de entrega</h3><p>El proyecto tiene un lapso de entrega de entre 10 y 15 días.</p>';
		modaltwo.innerHTML = '<i class="fa-solid fa-credit-card"></i><h3>Medios de pago</h3><p>Pay Pal / Bitcoin / USDT</p>';
		modalthree.innerHTML = '<i class="fa-solid fa-circle-dollar-to-slot"></i><h3>Plazos de pago</h3><p>50% al inicio del desarrollo web, 50% al finalizar el sitio.</p>';
		...
		iac.innerHTML = '<li id="iac"><strong>Diseñador de paginas web</strong> - Instituto Argentino de Computación (2004)</li>';
		project.textContent = 'Proyectos Freelance';
		projectp.textContent = 'Estoy abierto a cualquier propuesta para integrar un equipo de trabajo para proyectos freelance.';
		jobs.textContent = 'Ultimos proyectos finalizados';
		jobsp.textContent = 'Esta es una colección de mis últimos proyectos freelance';
		contactprojects.textContent = 'Contactame';
		contactprojectsp.textContent = 'Si te gustaron podés contactarme para comenzar un proyecto juntos.';
	   ...
	   element1.innerHTML = '<p id="element1">Hello! &#128075; Iam <span>Juan</span>, web development from Argentina. Interested design, Criptos, Startups and Marketing. Welcome to my web site.</p>';
	   home.textContent = 'Home';
	   aboutme.textContent = 'About me';
	   portfolio.textContent = 'Portfolio';
	   business.textContent = 'Do you have a business? Do you want to offer your services with a website?';
	   make.innerHTML = '<p id="make">Creating a website for this purpose requires always being updated and accompanying the client in order to achieve the best result. <br> <br>The projects are carried out so that they are adaptable to all mobile devices  <strong>( 100% Responsive )</strong>.</p>';
	   one.textContent = 'Web Design';
	   two.textContent = 'Development';
	   profesional.innerHTML = '<p>Professional development for startups and entrepreneurs. <br><br> We also offer <strong>web hosting</strong> services.</p>';
	   three.innerHTML = '<i class="fa-solid fa-arrow-up"></i>See more';
	   together.textContent = 'Do we work together?';
		togethertext.innerHTML = '<p id="togethertext">We will contact you shortly to start the project together.<strong> Contact me</strong>.</p>';
		...
		errortext.innerHTML = '<i class="fas fa-exclamation-triangle"></i> Please fill out the form correctly.';
		modalone.innerHTML = '<i class="fa-solid fa-calendar-days"></i><h3>Lead Time</h3><p>The project has a lead time of between 10 and 15 days</p>';
		modaltwo.innerHTML = '<i class="fa-solid fa-credit-card"></i><h3>Payment methods</h3><p>Pay Pal / Bitcoin / USDT</p>';
		modalthree.innerHTML = '<i class="fa-solid fa-circle-dollar-to-slot"></i><h3>Payment terms</h3><p>50% at the start of web development, 50 % at the end of the site</p>';
		...
		iac.innerHTML = '<li id="iac"><strong>Web designer</strong> - Instituto Argentino de Computación (2004)</li>';
		project.textContent = 'Freelance Projects';
		projectp.textContent = 'I am open to any proposal to integrate a work team for freelance projects.';
		...
		contactprojectsp.textContent = 'If you liked them you can contact me to start a project together.';
	}
  }
